...
`main` ブランチは EasyNote の元になるオープンソースコードです。コードを読み、
変更し、自分向けの EasyNote を継続的に作っていきたい人向けです。

このブランチはソースコードの更新をそのまま追うため、未完成または不安定な内容を含む場合があります。日常利用版として使うことはおすすめしません。
